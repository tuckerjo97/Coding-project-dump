...
def constructBayesNet(gameState):
    ...
    for house in HOUSE_VARS:
        ...
        edges += [(X_POS_VAR, house)]
        edges += [(Y_POS_VAR, house)]
        for housePos in gameState.getPossibleHouses():
            for obsPos in gameState.getHouseWalls(housePos):
                edges += [(house, OBS_VAR_TEMPLATE % obsPos)] 
        ...
        for walls in gameState.getHouseWalls(house):
            ...
            for assignment in obsFactor.getAllPossibleAssignmentDicts():
                if house == bottomLeftPos: 
                    if 'bottomLeft' != assignment['ghostHouse'] and 'bottomLeft' != assignment['foodHouse']:
                        ...
                        obsFactor.setProbability(assignment, 0)
                    elif 'bottomLeft' == assignment['foodHouse']:
                    ...
                    elif 'bottomLeft' == assignment['ghostHouse']:
                        if assignment[obsVar] == 'red':
                    ...
                    if 'topLeft' != assignment['ghostHouse'] and 'topLeft' != assignment['foodHouse']:
                        if assignment[obsVar] == NO_OBS_VAL:
                            obsFactor.setProbability(assignment, 1)
                        else:
                            obsFactor.setProbability(assignment, 0)
                        ...
                        obsFactor.setProbability(assignment, 0)
                    elif 'topLeft' == assignment['foodHouse']:
                        if assignment[obsVar] == 'red':
                            obsFactor.setProbability(assignment, PROB_FOOD_RED)
                        else:
                            obsFactor.setProbability(assignment, 1 - PROB_FOOD_RED)
                    elif 'topLeft' == assignment['ghostHouse']:
                        if assignment[obsVar] == 'red':
                            obsFactor.setProbability(assignment, PROB_GHOST_RED)
                        else:
                            obsFactor.setProbability(assignment, 1 - PROB_GHOST_RED)
                elif house == bottomRightPos:
                    if 'bottomRight' != assignment['ghostHouse'] and 'bottomRight' != assignment['foodHouse']:
                        if assignment[obsVar] == NO_OBS_VAL:
                            obsFactor.setProbability(assignment, 1)
                        else:
                            obsFactor.setProbability(assignment, 0)
                        ...
                        obsFactor.setProbability(assignment, 0)
                    elif 'bottomRight' == assignment['foodHouse']:
                        if assignment[obsVar] == 'red':
                            obsFactor.setProbability(assignment, PROB_FOOD_RED)
                        else:
                            obsFactor.setProbability(assignment, 1 - PROB_FOOD_RED)
                    elif 'bottomRight' == assignment['ghostHouse']:
                        if assignment[obsVar] == 'red':
                            obsFactor.setProbability(assignment, PROB_GHOST_RED)
                        else:
                            obsFactor.setProbability(assignment, 1 - PROB_GHOST_RED)
                elif house == topRightPos:
                    if 'topRight' != assignment['ghostHouse'] and 'topRight' != assignment['foodHouse']:
                        if assignment[obsVar] == NO_OBS_VAL:
                            obsFactor.setProbability(assignment, 1)
                        else:
                            obsFactor.setProbability(assignment, 0)
                        ...
                        obsFactor.setProbability(assignment, 0)
                    elif 'topRight' == assignment['foodHouse']:
                        if assignment[obsVar] == 'red':
                            obsFactor.setProbability(assignment, PROB_FOOD_RED)
                        else:
                            obsFactor.setProbability(assignment, 1 - PROB_FOOD_RED)
                    elif 'topRight' == assignment['ghostHouse']:
                        if assignment[obsVar] == 'red':
                            obsFactor.setProbability(assignment, PROB_GHOST_RED)
                        else:
                            obsFactor.setProbability(assignment, 1 - PROB_GHOST_RED)
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    sortedList = sorted(factor.getAllPossibleAssignmentDicts(), key=lambda a: a['foodHouse'])
    val = sortedList[0]['foodHouse']
    prob = factor.getProbability(sortedList[0])
    for assignment in sortedList[1:]:
        if factor.getProbability(assignment) >= prob:
            prob = factor.getProbability(assignment)
            val = assignment['foodHouse']
    
    return {FOOD_HOUSE_VAR: val}
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        for assignment in factor.getAllPossibleAssignmentDicts():
            if assignment['foodHouse'] == 'topLeft' and assignment['ghostHouse'] == 'topRight':
                prob1 += factor.getProbability(assignment)
            elif assignment['foodHouse'] == 'topRight' and assignment['ghostHouse'] == 'topLeft':
                prob2 += factor.getProbability(assignment)

        print prob1, prob2, factor
        
        return prob1 * GHOST_COLLISION_REWARD + prob2 * WON_GAME_REWARD, prob1 * GHOST_COLLISION_REWARD + prob2 * WON_GAME_REWARD
