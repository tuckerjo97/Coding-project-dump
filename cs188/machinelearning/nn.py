...
class Graph(object):
    ...
    def add(self, node):
        ...
        self.data[node] = [inputs, output, gradient]
    ...
    def backprop(self):
        ...
        nodeList = self.nodes[::-1]
        self.data[loss_node][2] = 1.0
        for node in nodeList:
            for i in range(len(node.get_parents())):
                parent = node.get_parents()[i]
                self.data[parent][2] += node.backward(self.get_inputs(node), self.get_gradient(node))[i]
...
class MatrixMultiply(FunctionNode):
    ...
    @staticmethod
    def backward(inputs, gradient):
        return [np.array(np.dot(gradient, inputs[1].T)), np.array(np.dot(inputs[0].T, gradient))]
...
class ReLU(FunctionNode):
    ...
    def forward(inputs):
        ...
        return np.maximum(inputs[0], zeros)

    @staticmethod
    def backward(inputs, gradient):
        shape = np.shape(gradient)
        zeros = np.zeros(shape)
        return [np.where((inputs[0] > 0), gradient, zeros)]
        
        # return [np.array(np.multiply(ReLU.forward(inputs), gradient))]
...
class SquareLoss(FunctionNode):
    ...
    @staticmethod
    def forward(inputs):
        subtract = inputs[0] - inputs[1]
        square = np.multiply(subtract, subtract)
        half = square / 2
        return np.mean(half)

    @staticmethod
    def backward(inputs, gradient):
        mean = gradient/len(inputs[0])
        return [np.array(inputs[0] - inputs[1]) * mean, np.array(inputs[1] - inputs[0]) * mean]
