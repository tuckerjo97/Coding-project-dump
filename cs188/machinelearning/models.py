...
class RegressionModel(Model):
    ...
    def __init__(self):
        ...
        self.w1 = nn.Variable(1, self.layer_size)
        self.w2 = nn.Variable(self.layer_size, 1)
        self.b1 = nn.Variable(self.layer_size)
    ...
    def run(self, x, y=None):
        ...
        relu_xw1_plus_b1 = nn.ReLU(self.graph, xw1_plus_b1)
        xw2 = nn.MatrixMultiply(self.graph, relu_xw1_plus_b1, self.w2)
        xw2_plus_b2 = nn.MatrixVectorAdd(self.graph, xw2, self.b2)
        ...
        if y is not None:
            ...
            nn.SquareLoss(self.graph, xw2_plus_b2, input_y)
            return self.graph

        else:
            # At test time, the correct output is unknown.
            # You should instead return your model's prediction as a numpy array
            return self.graph.get_output(xw2_plus_b2)
...
class OddRegressionModel(Model):
    ...
    def run(self, x, y=None):
        ...
        negate = np.ones((1,1)) * -1
        negatives = np.dot(x, negate)
        ninput_x = nn.Input(self.graph, negatives)

        nxw1 = nn.MatrixMultiply(self.graph, ninput_x, self.w1)
        nxw1_plus_b1 = nn.MatrixVectorAdd(self.graph, nxw1, self.b1)
        nrelu_xw1_plus_b1 = nn.ReLU(self.graph, nxw1_plus_b1)
        nxw2 = nn.MatrixMultiply(self.graph, nrelu_xw1_plus_b1, self.w2)
        nxw2_plus_b2 = nn.MatrixVectorAdd(self.graph, nxw2, self.b2)
        negate = nn.Input(self.graph, negate)
        nxw2_plus_b2 = nn.MatrixMultiply(self.graph, nxw2_plus_b2, negate)

        x_sub = nn.Add(self.graph, xw2_plus_b2, nxw2_plus_b2)
        ...
        if y is not None:
            ...
            # added to the graph.
            input_y = nn.Input(self.graph, y)
            nn.SquareLoss(self.graph, x_sub, input_y)
            return self.graph
        else:
            # At test time, the correct output is unknown.
            # You should instead return your model's prediction as a numpy array
            return self.graph.get_output(x_sub)
