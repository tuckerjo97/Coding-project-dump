...
void v_add_optimized_adjacent(double* x, double* y, double* z) {
     #pragma omp parallel
	{
		...
		for(int i = curr_thread; i < ARRAY_SIZE; i += num_threads)
			z[i] = x[i] + y[i];
...
void v_add_optimized_chunks(double* x, double* y, double* z) {
          #pragma omp parallel
	{
		...
		for(int i = size * curr_thread; i < (curr_thread + 1) * size && i < ARRAY_SIZE; i++)
			z[i] = x[i] + y[i];
		

		if(curr_thread == 0){
			if(ARRAY_SIZE % num_threads != 0){
				for(i = ARRAY_SIZE - (ARRAY_SIZE % num_threads); i < ARRAY_SIZE; i++){
					z[i] = x[i] + y[i];
				}
			}
		}
